  ...
  return (
    <div className='flex flex-col w-full
      ...
      <div
        ...
        rounded-4xl border-5 lg:border-10 border-blueblack
        ...
        <h3 className="w-full text-base lg:text-xl 2xl:text-2xl comic-relief-bold text-shadow-[-1px_3px_0px_#0F1B24] lg:text-shadow-[-2px_6px_0px_#0F1B24]">“{quote}”</h3>
      </div>
      <div className="flex flex-col 2xl:flex-row gap-6 h-auto">
        {children.map((trait, index) => (
          ...
          rounded-4xl border-5 lg:border-10 border-blueblack
          drop-shadow-[-12px_15px_25px_#0F1B24]">
            <h3 className="text-xl 2xl:text-3xl uppercase tracking-wider text-shadow-[-1px_3px_0px_#0F1B24] lg:text-shadow-[-2px_4px_0px_#0F1B24]">{trait.title}</h3>
            <p className="text-sm lg:text-base 2xl:text-xl comic-relief-bold text-shadow-[-1px_3px_0px_#0F1B24] lg:text-shadow-[-2px_3px_0px_#0F1B24]">{trait.description}</p>
          </div>
        ))}
      </div>
    </div>
