export default function FeaturesCard({ item }) {
  const { title, subtitle, description, image, color = 'bg-hijaubaru-reverse' } = item;

  return (
    <div className={`flex flex-col text-white text-shadow-[-2px_3px_0px_#0F1B24] items-center 
      py-5 2xl:py-10 
      px-5 2xl:px-15 
      w-full h-130 2xl:h-160 
      ${color} border-blueblack border-5 rounded-3xl 
      gap-3 2xl:gap-6 
